...
ch_logger = Logger(q_log, base_level=logging.DEBUG, filename="")

th_log = logging.getLogger("UserTaskHandler")
th_logger = Logger(q_log, base_level=logging.DEBUG, filename="")
...
class UserCommandHandler:
    ...
    def set_user_command(self, user_id: int, cmd: UserCmd) -> None:
        ch_logger.log(logging.DEBUG, message=f"set user {user_id=}: {cmd=}")
    ...
    def get_user_command(self, user_id) -> Union[UserCmd, None]:
        ...
        if user_id in self._c_dict:
            ch_logger.log(logging.INFO, message=f"get user return for {user_id=}")
